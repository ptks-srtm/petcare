...
export function ConsultationContextSummary({ request }: { request: PetConsultationRequest }) {
	...
	return (
		<section aria-labelledby="consult-context-title" className="pc-card p-5">
			...
			{request.recentLogs.length === 0 && <div className="mt-3 rounded-2xl border border-dashed border-border-soft bg-white p-4"><p className="text-sm font-semibold text-slate-700">直近7日間の記録はありません。</p><p className="mt-1 text-xs leading-relaxed text-slate-500">相談はできますが、うんち・ごはん・さんぽを記録すると、より具体的な情報をAIへ共有できます。</p></div>}
