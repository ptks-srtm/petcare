...
export function ConsultationForm({ topic, concern, isSubmitting, error, onTopicChange, onConcernChange, onSubmit }: {
	...
	return (
		<form onSubmit={handleSubmit} className="pc-card p-5" aria-labelledby="consult-form-title">
			<h3 id="consult-form-title" className="text-lg font-semibold text-slate-800">相談内容</h3>
			...
			<button type="submit" disabled={!canSubmit} className="pc-button-primary mt-5 w-full px-5 text-base">{isSubmitting ? '記録を確認しています…' : '相談内容を整理する'}</button><p aria-live="polite" className="sr-only">{isSubmitting ? '記録を確認しています' : ''}</p>
		</form>
	);
}
