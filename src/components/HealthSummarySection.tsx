...
export function HealthSummarySection({ today, weekly, insight, section }: HealthSummarySectionProps) {
	...
	const hasWeeklyRecords = weekly.poopCount > 0 || weekly.mealCount > 0 || weekly.walkCount > 0;
	...
	return (
		<section aria-labelledby="weekly-summary-title" className="mt-9">
			...
			{hasWeeklyRecords ? <div className="grid grid-cols-3 gap-2.5">
			...
			</div> : <EmptyState title="まだ記録がありません" description="さんぽ・ごはん・うんちを記録すると、ここに直近7日間の様子が表示されます。" action={<a href="#new-log-title" className="pc-button-secondary px-4 text-sm">記録エリアを見る</a>} />}
